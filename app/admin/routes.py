...
class AdminOnlyView(ModelView):
    ...
    def on_model_change(self, form, model, is_created):
        ...
        if form.is_validated.data:  # Se il checkbox è selezionato
            model.is_validated = 1  # Impostiamo la stringa 'True'
        else:  # Se il checkbox non è selezionato
            model.is_validated = 0  # Impostiamo la stringa 'False'

        if form.light_theme.data:  # Per il checkbox light_theme
            model.light_theme = 1  # Impostiamo la stringa 'True'
        else:
            model.light_theme = 0  # Impostiamo la stringa 'False'
